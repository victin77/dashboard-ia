...
function Pill({ icon: Icon, label, value, tone='violet', variants }) {
  const map = {
  ...
  return (
    <motion.div
      ...
      className={`rounded-3xl p-5 bg-gradient-to-r ${map[tone]} shadow-xl`}
    >
      ...
      {open && (
        ...
        >
          ...
          <motion.div
            role="dialog"
            aria-modal="true"
            className={`relative w-full ${widthClass} rounded-3xl bg-white dark:bg-slate-950 border border-slate-200/60 dark:border-white/10 shadow-2xl overflow-hidden`}
            initial={reduce ? { opacity: 1 } : { opacity: 0, y: 14, scale: 0.98, filter: 'blur(6px)' }}
            animate={{ opacity: 1, y: 0, scale: 1, filter: 'blur(0px)' }}
            exit={reduce ? { opacity: 0 } : { opacity: 0, y: -10, scale: 0.98, filter: 'blur(6px)' }}
...
export default function Dashboard({ user, onLogout }) {
  ...
  const pageTransition = useMemo(() => (
    reduceMotion ? { duration: 0.01 } : { type: 'spring', stiffness: 260, damping: 24 }
  ), [reduceMotion]);
              ...
              <div className="space-y-8">
                {summary && (
                  <div className="space-y-3">
                    ...
                    >
                      <Pill variants={cardVariant} icon={FileText} label="Vendas hoje" value={`${summary.today?.sales_count || 0}`} tone="cyan" />
                      <Pill variants={cardVariant} icon={BarChart3} label="Comissão hoje" value={fmtBRL(summary.today?.commission_total || 0)} tone="emerald" />
                      <Pill variants={cardVariant} icon={FileText} label="Vendas (7 dias)" value={`${summary.last7?.sales_count || 0}`} tone="violet" />
                      <Pill variants={cardVariant} icon={BarChart3} label="Comissão (mês)" value={fmtBRL(summary.month?.commission_total || 0)} tone="violet" />
                      <Pill variants={cardVariant} icon={BarChart3} label="Parcelas pendentes" value={`${summary.installments?.pending || 0}`} tone="amber" />
                      <Pill variants={cardVariant} icon={BarChart3} label="Parcelas atrasadas" value={`${summary.installments?.overdue || 0}`} tone="rose" />
                    </motion.div>
                  </div>
                )}
                ...
                >
                  <Pill variants={cardVariant} icon={BarChart3} label="Comissão total" value={fmtBRL(kpis.total)} tone="violet" />
                  <Pill variants={cardVariant} icon={BarChart3} label="Pago" value={fmtBRL(kpis.paid)} tone="emerald" />
                  <Pill variants={cardVariant} icon={BarChart3} label="Pendente/Atrasado" value={fmtBRL(kpis.pending + kpis.overdue)} tone="amber" />
                </motion.div>
